...
contract CollectionManager is AccessControl, Pausable {
  ...
  function stakeNFT(uint256 tokenId) external {
    stakingContract.stake(msg.sender, tokenId);
  }

  function claimRewards() external {
    stakingContract.claimRewards(msg.sender);
  }
  ...
  function listForSale(address nftContract, uint256 tokenId, uint256 price, uint256 amount) external {
    ...
    if (isERC721) {
      marketplaceContract.listERC721ForSale(msg.sender, tokenId, price);
    } else {
      require(IERC165(nftContract).supportsInterface(type(IERC1155).interfaceId), 'Not ERC721 or ERC1155');
      marketplaceContract.listERC1155ForSale(msg.sender, tokenId, amount, price);
    }
  }

  function getUserNFTs(address user) external view returns (uint256[] memory) {
    return _erc721.tokensOfOwner(user);
  }
